...
# Requires: libubox, relayd (compiled from openwrt git source)
...
cleanup()
{
	iptables -D FORWARD -i $WIFI_IF -j ACCEPT
	iptables -D FORWARD -i $AP_IF -j ACCEPT
...
iptables -I FORWARD -i $AP_IF -j ACCEPT
iptables -I FORWARD -i $WIFI_IF -j ACCEPT
